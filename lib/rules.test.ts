...
describe('allocation rules',()=>{
 it('allows Base 1 -> Plate back-to-back',()=>expect(validateUmpire(umpire,games,[a('g1','Base 1'),a('g2','Plate')])).toHaveLength(0))
 it('rejects Plate -> Base 1',()=>expect(validateUmpire(umpire,games,[a('g1','Plate'),a('g2','Base 1')]).some(v=>v.rule==='PLATE_BREAK'&&v.severity==='hard')).toBe(true))
 ...
 it('allows Plate -> OFF -> Base 1',()=>expect(validateUmpire(umpire,games,[a('g1','Plate'),a('g3','Base 1')])).toHaveLength(0))
 it('rejects Base 1 -> Base 1',()=>expect(validateUmpire(umpire,games,[a('g1','Base 1'),a('g2','Base 1')]).some(v=>v.rule==='BACK_TO_BACK'&&v.severity==='hard')).toBe(true))
 it('rejects a fourth game',()=>expect(validateUmpire(umpire,games,[a('g1','Base 1'),a('g2','Plate'),a('g3','Base 1'),a('g4','Base 1')]).some(v=>v.rule==='MAX_GAMES'&&v.severity==='hard')).toBe(true))
 ...
 it('does not count another day towards the daily maximum',()=>{const nextDay:Game={...games[3],id:'g5',number:5,date:'2026-08-31'};expect(validateUmpire(umpire,[...games,nextDay],[a('g1','Base 1'),a('g2','Plate'),a('g3','Base 1'),{...a('g4','Base 1'),gameId:'g5'}]).some(v=>v.rule==='MAX_GAMES')).toBe(false)})
})
