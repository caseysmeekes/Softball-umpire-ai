...
export function validateUmpire(umpire: Umpire, games: Game[], assignments: Assignment[]): Violation[] {
  ...
  for(let i=0;i<mine.length-1;i++){
    ...
    else if(next.position!=='Plate') issues.push({umpireId:umpire.id,rule:'BACK_TO_BACK',severity:'hard',gameId:next.gameId,message:`Rule 2: back-to-back games must be Base → Plate. Game ${current.game.number} is Base, so Game ${next.game.number} must be Plate.`})
  }
...
export function allocate(games:Game[],umpires:Umpire[]){ return allocateUnlocked(games,umpires,[]) }

export function allocateUnlocked(games:Game[],umpires:Umpire[],lockedAssignments:Assignment[]){
  ...
  for(const game of orderedGames(games)) for(const position of game.positions){
    ...
    else unallocated.push({game,position,reasons:['No available umpire satisfies all hard rules. Locked assignments were preserved.']})
  }
