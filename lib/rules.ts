...
export function validateUmpire(umpire: Umpire, games: Game[], assignments: Assignment[], activeRuleIds?: string[]): Violation[] {
  ...
  const mine = assignments.filter(a => a.umpireId === umpire.id)
  ...
  for (const item of mine) {
    ...
    ids.add(item.gameId); byDate.set(item.game.date, ids)
  }
  if (rules.has('max-games')) for (const [date, ids] of byDate) if (ids.size > umpire.maxGames)
    issues.push({ umpireId: umpire.id, rule: 'MAX_GAMES', severity: 'hard', message: `Rule 1: ${umpire.name} has ${ids.size} games on ${date}, exceeding the maximum of ${umpire.maxGames}.` })
  if (rules.has('no-double-booking')) for (let i = 0; i < mine.length; i++) for (let j = i + 1; j < mine.length; j++) {
    const a = mine[i].game, b = mine[j].game
    if (a.date === b.date && start(b) < new Date(`${a.date}T${a.end}:00`).getTime() && start(a) < new Date(`${b.date}T${b.end}:00`).getTime())
      issues.push({ umpireId: umpire.id, rule: 'NO_DOUBLE_BOOKING', severity: 'hard', gameId: mine[j].gameId, message: `${umpire.name} is double-booked between Game ${a.number} and Game ${b.number}.` })
  }
  for (let i = 0; i < mine.length - 1; i++) {
    ...
    if (rules.has('plate-break') && current.position === 'Plate')
      issues.push({ umpireId: umpire.id, rule: 'PLATE_BREAK', severity: 'hard', gameId: next.gameId, message: `Rule 3: Plate on Game ${current.game.number} requires the next scheduled game off.` })
    if (rules.has('back-to-back') && current.position !== 'Plate' && next.position !== 'Plate')
      issues.push({ umpireId: umpire.id, rule: 'BACK_TO_BACK', severity: 'hard', gameId: next.gameId, message: `Rule 2: back-to-back games must be Base → Plate. Game ${current.game.number} is Base, so Game ${next.game.number} must be Plate.` })
  }
  if (rules.has('one-plate')) {
    const plates = new Map<string, number>()
    for (const item of mine) if (item.position === 'Plate') plates.set(item.game.date, (plates.get(item.game.date) || 0) + 1)
    for (const [date, count] of plates) if (count > 1)
      issues.push({ umpireId: umpire.id, rule: 'ONE_PLATE', severity: 'soft', message: `Soft rule: ${umpire.name} has ${count} Plate assignments on ${date}. Aim for no more than one Plate game per day.` })
  }
  return issues
}

export function canAssign(umpire: Umpire, game: Game, position: Position, games: Game[], assignments: Assignment[], activeRuleIds?: string[]) {
  ...
  return !validateUmpire(umpire, games, trial, activeRuleIds).some(v => v.severity === 'hard' && (v.gameId === game.id || v.rule === 'MAX_GAMES' || v.rule === 'NO_DOUBLE_BOOKING' || v.rule === 'BACK_TO_BACK' || v.rule === 'PLATE_BREAK'))
}
...
export function allocateUnlocked(games: Game[], umpires: Umpire[], lockedAssignments: Assignment[], activeRuleIds?: string[]) {
  const rules = enabled(activeRuleIds), assignments = [...lockedAssignments]
  const unallocated: { game: Game; position: Position; reasons: string[] }[] = []
  for (const game of orderedGames(games)) for (const position of game.positions) {
    ...
    const candidates = umpires.filter(u => canAssign(u, game, position, games, assignments, activeRuleIds)).sort((a, b) => {
      const score = (u: Umpire) => {
        const mine = assignments.filter(x => x.umpireId === u.id), count = new Set(mine.map(x => x.gameId)).size, plates = mine.filter(x => x.position === 'Plate').length
